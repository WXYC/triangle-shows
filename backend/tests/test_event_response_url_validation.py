"""Unit tests for EventResponse's ticket_url/image_url normalization (issue #94).
...
them at ingestion (tests/test_scraped_event_url_validation.py), but EventResponse
is what every API consumer — the web client and the Backend-Service "On Tour"
reader — actually observes, regardless of how a row reached the database (a row
written before the ingestion-time gate existed, or by any future path that
doesn't go through ScrapedEvent). Both gates must independently null out a
...
def test_attribute_breakout_payload_that_still_starts_with_https_is_not_this_layers_job():
    # The prefix check alone can't stop a `"` later in the string (that's the
    # frontend _h() escaping's job, in frontend/js/modal.js) — a value that starts
    # with a real https:// scheme is a well-formed absolute URL as far as this
    # validator is concerned and passes through unchanged. Documented here so a
    # future reader doesn't mistake this gate for the whole fix.
