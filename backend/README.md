...
| A future scrape-health digest embeds the same evaluator `detail` in the text posted to a chat webhook | Already scrubbed by the evaluator above, one sanitization point for both consumers; `app.observability.send_alert`'s own `redact_credentials` pass is a second, idempotent backstop at the sink |
