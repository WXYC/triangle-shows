...
The versioned `/api/v1` endpoints (`/api/v1/events`, `/api/v1/events/{id}`, `/api/v1/venues`, `/api/v1/health`, `/api/v1/site`, `/api/v1/health/scrapers`) are the canonical, client-agnostic contract. The unversioned `/api/events`, `/api/venues`, and `/api/health` routes are deprecated aliases kept for the current web client; `/feeds/events.ics` is the iCal subscription feed. Shared fetch/filter/de-duplication logic lives in `app/services/events_query.py` (and `app/services/scrape_health_query.py` for the scrape-health surfaces) with shared route helpers in `app/api/common.py`, so every surface serves the same data.
...
`app.scheduler.scrape_health_digest_job` (issue #86 part 2) is a daily consumer of the pure evaluator in `app/services/scrape_health.py` — it reuses `evaluate_venue_health` exactly as `GET /api/v1/health/scrapers` does, unmodified, rather than deriving verdicts a second way. It reads its rows through the same shared fetch, `app/services/scrape_health_query.py`, for the same reason: a job that computed "broken" from a different row cap or a different baseline window than the endpoint reports would leave an operator no way to tell which of the two is wrong.
...
- **Transition-only alerting.** The job alerts only when a venue's broken/not-broken state *changes* — not every day a venue stays broken, and not at all when nothing changed anywhere. "Broken" means the evaluator's `status` is `warning` or `critical`; `ok` and `unknown` both count as not-broken. A venue that just crossed from one to the other is reported; a venue that has held the same state since the previous run is silent. Grouping `unknown` with `ok` is deliberate — the evaluator returns it only for a venue with no visible scrape history at all, and never-scraped is not broken. The alternative would page once for every venue added to `venues.toml`, in the window between its row existing and its first scrape finishing. A venue whose scrapes are genuinely failing is unaffected by that choice: failed attempts are visible rows, so it reaches `critical` on the consecutive-failure signal as soon as the streak fills the window.
...
- **A skipped run drops that day's transitions — by design, and the endpoint is the backstop.** The replay's reference point is "24 hours ago", not "whenever this job last succeeded", so if the 7 AM run doesn't happen (a redeploy across the trigger, an APScheduler misfire, the job itself raising), the next morning's run compares against an instant at which the venue was *already* broken and reports no transition. Widening the lookback isn't a fix — a 48-hour window re-pages every break on two consecutive mornings. The durable, miss-proof view of the same verdicts is `GET /api/v1/health/scrapers`, which recomputes from `ScrapeLog` on every request and is exactly what an uptime monitor should poll; the digest is a push channel for *changes*, not the system of record. If missed runs turn out to be common in practice, the fix is the durable `scrape_health_state` table issue #86 holds in reserve, not a wider replay.
