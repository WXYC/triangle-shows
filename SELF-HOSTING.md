...
2. **Push delivery to a chat webhook.** Set `ALERT_WEBHOOK_URL` to a Slack, Mattermost, or Discord (append `/slack` to a Discord webhook URL) incoming-webhook URL and a plain JSON `{"text": ...}` POST is sent. This channel is for **scrape-health digests only** — not for internal errors, which always go to stderr (and, if configured, the exception tracker below) regardless of this setting. Leave it empty and the digest is logged instead of posted.

   The digest itself runs once a day, at 7 AM in the region's market timezone (right after the morning scrape wave, so its own results are already recorded), and it alerts only on a **change**: a venue that just started failing, or a venue that just recovered. A venue that has been broken (or healthy) since the previous day's digest stays silent rather than re-paging every morning — see `backend/README.md`'s "Scrape-health digest" section for exactly how that transition is computed.
